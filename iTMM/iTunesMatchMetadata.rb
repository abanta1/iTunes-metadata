...
puts "*" * 80
print "*" * 25
print "   iTunes Metadata Matching   "
puts "*" * 25
print "*" * 25
print "       by Anthony Banta       "
puts "*" * 25
puts "*" * 80
puts "*" * 80
puts " " * 80

tspaces = 80 - (15 + iTunes_tracks.count.to_s.length)
tlspaces = tspaces / 2
tlspace = " " * tlspaces
trspaces = tspaces - tlspaces
trspace = " " * trspaces

printf("%sReading %i tracks", tlspace, iTunes_tracks.count)
iTunes_tracks.each do | iTunes_track |
  ...
  begin
    if iTunes_track.kind.get == 'Matched AAC audio file' && iTunes_track.grouping.get != 'USA'
	  ...
	  unless track.valid?
        tracks << track
        printf " .\033[D\033[D" if ((tracks.count % WORK_SIZE) % 4 == 2)
        printf "  \033[D\033[D" if ((tracks.count % WORK_SIZE) % 4 == 0)
      end
    end
...
fspaces = 80 - (21 + iTunes_tracks.count.to_s.length)
flspaces = fspaces / 2
flspace = " " * flspaces
frspaces = fspaces - flspaces
frspace = " " * frspaces


if tracks.count == 0
	print " " * 30
	puts "Didn't find any international matched tracks"
	exit
else
#	print " " * 25
	printf("%sFound %i matched tracks\n\n\n", flspace, tracks.count)
end

  $eqcount = ""
  $spacecount = " " * 20
  $spacenum = 0
  $totaracks = tracks.count
  $spc = " "
  
	if tracks.count < 11
		$numrack = 1.0 / 2
	elsif tracks.count < 101
		$numrack = 5
	elsif tracks.count < 1001
		$numrack = 50
	elsif tracks.count < 10001
		$numrack = 500
	else
		$numrack = 5000
	end
...
cname = get_sfid(country)

#Querying US store for 17560 tracks [====================] 10 found 10 updated
#  printf( "    Pillaging the %s for%5i tracks [#{$eqcount}#{$spacecount}]\r", country, tracks.count)
#	printf( "    Pillaging the %s for%5i tracks [#{$eqcount}#{$spacecount}]\r", cname, tracks.count)
cspaces = 80 - (10 + cname.length)
clspaces = cspaces / 2
clspace = " " * clspaces
crspaces = cspaces - clspaces
crspace = " " * crspaces

	printf( "%s%sPillaging %s%s%s", "\r", clspace, cname, crspace, "\r")
    ...
    iTunesHash['results'].each do | result |
      ...
      subtracks.each do | track |  
        ...
        if result_id == track.iTunes_id
      	...
      	$iter += 1
      	if $iter % $numrack == 0
			printf( "%sPillaging %s%s\n", clspace, cname, crspace)
			printf( "%32sfor%5i tracks\n", $spc, tracks.count) 
			$spacenum = 20 - $iter / $numrack
			if $spacenum > 0
			$spacecount = " " * $spacenum
			$eqcount = "=" * (20 - $spacenum)
			elsif $spacenum == 0
			...
			printf( "%16s [%s%s]%s%s", $spc, $eqcount, $spacecount, "\033[A", "\033[A")
		end
...
#            `rm .image.png`
  ...
  printf( "%sPillaging %s%s\n", clspace, cname, crspace)
  printf( "%32sfor%5i tracks\n", $spc, tracks.count)
  printf( "%16s [%s%s]", $spc, $eqcount, $spacecount )
  printf( " %5i found ", found)

#  if counter > 0
	printf( "%5d updated\r", counter)

	printf( "\033[A\033[A")

#  else
#    printf( "%5d updated\033[A\033[A", counter) 
#  end
...
uspaces = 80 - (9 + (iTunes_tracks.count.to_s.length * 2))
ulspaces = uspaces / 2
ulspace = " " * ulspaces
urspaces = uspaces - ulspaces
urspace = " " * urspaces

rspaces = 80 - (33 + iTunes_tracks.count.to_s.length)
rlspaces = rspaces / 2
rlspace = " " * rlspaces
rrspaces = rspaces - rlspaces
rrspace = " " * rrspaces

if tracks.count > 0
#print "i#{$spc}i"
	print "\r"
    printf( "%-29sPillaged all countries%s\n", $spc, $spc * 29)
    printf( "%s%i/%i updated%s", ulspace, found, $totaracks, urspace)

#    printf( "\n   Pillaged all countries,               [====================] #{found}/%i updated     ", $totaracks)  
#	puts "Couldn't find meatadata for #{tracks.count} tracks"
	print "\n"
	printf("%sAdding remaining %i tracks to vault%s\n", rlspace, tracks.count, rrspace)
