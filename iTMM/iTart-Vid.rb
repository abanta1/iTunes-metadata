...
username="user"
home = Dir.home
#Dir.chdir("/Volumes/E$/Movies")
Dir.chdir("/Users/"""$username"""/uTorrent/Add")
curdir = Dir.pwd
...
#    f="/Volumes/Data/Add/Furious.6.2013.720p.BluRay.x264.YIFY.m4v"
#    puts f
...
# Filename without extension    
    filenoext=fileext.gsub(/\.m4v/, '').gsub(/\.mp4/, '').gsub(/\.mkv/, '')
#    print filenoext
#    puts " + filenoext"
...
# Is it tv or movie
	entityv="movie"
#	opfile="#{home}/Desktop/#{noyear}.jpg"
	opfile="#{filepat}/#{noyear}.jpg"
#	print opfile
#	puts " + opfile"
...
# Set to TV show, set episode ID and ssn num
	unless epsid.nil?
	  ...
	  entityv="tvSeason"
#	  opfile="#{home}/Desktop/#{noyear}.#{ssnnum}.jpg"
	  opfile="#{filepat}/#{noyear}.#{ssnnum}.jpg"
#	  puts ssnnum + " ssn"
	end
	
#	 puts opfile
	
	fileshownameo=noyear.gsub(/[ \.]/, '+')
	fileshowname=fileshownameo.gsub("**", ':')
#	puts fileshowname
      ...
      fileshownamet=fileshowname.gsub(/[+]/, ' ')
      	...
      	if (result['trackName'].similar(fileshownamet) > 80) || ((result['trackName'].include? fileshownamet) == true)
      	  ...
      	  if (resyrlo..resyrhi) === resyear
      	  	arturl=result['artworkUrl100'].sub(/100x100/, '600x600')
      	  	print "Downloading art for #{fileshownamet}, "
      	  	open(opfile, 'w+') do |file|
			  file << open(arturl).read
			end
			puts " Done"
